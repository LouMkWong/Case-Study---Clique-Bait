  ````sql
    ...
    SELECT 
    	ROUND(AVG(page_viewed), 2) as average_page_viewed_per_visit,
    	ROUND(AVG(purchased), 2) as average_purchased_per_visit,
        ROUND(AVG(ad_clicked), 2) as average_ad_clicked_per_visit
    FROM SUMMARY2;
````
